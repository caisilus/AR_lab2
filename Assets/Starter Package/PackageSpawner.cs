...
public class PackageSpawner : MonoBehaviour
{
    ...
    public void SpawnPackage(ARPlane plane, Vector3 packagePosition)
    {
        var packageClone = GameObject.Instantiate(PackagePrefab);
        //packageClone.transform.position = FindRandomLocation(plane);
        packageClone.transform.position = packagePosition;
    ...
    private void Update()
    {
        ...
        if (lockedPlane != null)
        {
            if (Package == null)
            {
                var randomPosition = FindRandomLocation(lockedPlane);
                SpawnPackage(lockedPlane, randomPosition);
            }
            else if (WasTapped())
            {
                ProcessTap();
    ...
    private void ProcessTap()
    {
        ...
        DrivingSurfaceManager.RaycastManager.Raycast(touchPosition, hits, TrackableType.PlaneWithinBounds);
        ...
        if (hit.HasValue)
        {
            SpawnPackage(lockedPlane, hit.Value.pose.position);
        }
    }
